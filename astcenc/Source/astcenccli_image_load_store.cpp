...
static const struct
{
    ...
    // LDR formats
    {2, "\xFF\xD8\0",  load_image_with_stb}, // jpg
    {8, "\x89\x50\x4E\x47\x0D\x0A\x1A\x0A\0",  load_png_with_wuffs}, // png
};

static const int loader_fmt_descr_count = sizeof(loader_fmt_descs) / sizeof(loader_fmt_descs[0]);
...
astcenc_image* TryLoadByFormat(const char* filename, bool y_flip, bool& is_hdr, unsigned int& component_count)
{
    const std::streamsize readdatalen = 8;
    ...
    char data[readdatalen];
    file.read(data, readdatalen);
    for (int i = 0; i < loader_fmt_descr_count; i++)
    {
        if (memcmp(data, loader_fmt_descs[i].magic, loader_fmt_descs[i].count) == 0)
        {
            return loader_fmt_descs[i].loader_func(filename, y_flip, is_hdr, component_count);
        }
    }
...
astcenc_image* load_ncimage(
    ...
    // [stdliu]根据文件内容加载文件，CDN图片存在扩展名不准的问题
    astcenc_image* pImage = TryLoadByFormat(filename, y_flip, is_hdr, component_count);
    if (pImage)
    {
        return pImage;
    }
