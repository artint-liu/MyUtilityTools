...
BOOL VertexPaint::ActivatePaint(BOOL bOnOff, BOOL bPick)
{
    if (bOnOff)
    {
        ...
        m_ip->SetCommandMode(&thePaintCommandMode);
    }
    else {
...
{
    int numPoints;
    iPaintButton->SetCheck(m_ip->GetCommandMode()->ID() == CID_PAINT &&
        !((PaintMouseProc*)m_ip->GetCommandMode()->MouseProc(&numPoints))->GetPickMode());
    iPickButton->SetCheck(m_ip->GetCommandMode()->ID() == CID_PAINT &&
        ((PaintMouseProc*)m_ip->GetCommandMode()->MouseProc(&numPoints))->GetPickMode());
    m_ip->GetModContexts(modContexts, m_nodeTab);
}
...
BOOL VertexPaint::IsValidNode(INode* node)
{
    ...
    for (int i = 0; i < m_nodeTab.Count(); i++) {
        if (m_nodeTab[i] == node) {
            return TRUE;
        }
    }
