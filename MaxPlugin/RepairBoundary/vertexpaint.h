...
#define VERTEXPAINT_CLASS_ID	Class_ID(0x663b6fab, 0x73f57385)
#define CID_PAINT						(CID_USER+0x439c)
#define NUMPALETTES					16
#define WM_POSTINIT					(WM_USER+0x1177)
//#define WM_UPDATENAME				(WM_USER+0x1178)
...
public:
		...
		void GetClassName(MSTR& s, bool localized) const override { s= localized ? TSTR(GetString(IDS_CLASS_NAME)) : _T("OldVertexPaint"); }  
...
public:
		...
		void SetReferenceNode(INode* pNode);
		void ResetTransform();
		void RepairBoundary();
	...
	private:
		...
		INodeTab		m_nodeTab;
		INode*			m_pReferenceNode = nullptr;
		HWND			hPaletteWnd[NUMPALETTES];
		float			fTint;
		ISpinnerControl *iTint;
		DWORD flags;

		std::vector<UserPoint> m_userPoint;
