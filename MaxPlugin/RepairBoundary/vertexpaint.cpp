...
#include "PickMode.h"
#include "utility.h"
#include "ResetNodeTransform.h"
#include "RepairBoundary.h"
...
static ModelPickMode pick_mode;
...
static INT_PTR CALLBACK VertexPaintDlgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        mod->iPaintButton->SetCheck(mod->m_ip->GetCommandMode()->ID() == CID_PAINT &&
            !((PaintMouseProc*)mod->m_ip->GetCommandMode()->MouseProc(&numPoints))->GetPickMode());
        ...
        mod->iPickButton->SetCheck(mod->m_ip->GetCommandMode()->ID() == CID_PAINT &&
            ((PaintMouseProc*)mod->m_ip->GetCommandMode()->MouseProc(&numPoints))->GetPickMode());
        ...
        SetDlgItemText(hWnd, IDC_SEL_OBJECTNAME, mod->m_pReferenceNode == nullptr ? _T("<None>") : mod->m_pReferenceNode->GetName());
        //EnableDlgItem(hWnd, IDC_RESET_TRANSFORM, (mod->m_pReferenceNode != 0));
        break;
    ...
    case WM_COMMAND:
        ...
        case IDC_SEL_OBJECT:
            ...
            pick_mode.Setup(mod, hWnd, IDC_SEL_OBJECTNAME);
            GetCOREInterface()->ClearPickMode();
            GetCOREInterface()->SetPickMode(&pick_mode);
            SetDlgItemText(hWnd, IDC_SEL_OBJECTNAME, _T("picking"));
            break;

        case IDC_RESET_TRANSFORM:
            mod->ResetTransform();
            break;

        case IDC_REPAIR_BOUNDARY:
            mod->RepairBoundary();
            break;
        }
        break;
    ...
    //case WM_UPDATENAME:
    //    SetDlgItemText(hWnd, wParam, (LPCWSTR)lParam);
    //    break;
...
IObjParam* VertexPaint::m_ip = NULL;
...
void VertexPaint::SetReferenceNode(INode* pNode)
{
    m_pReferenceNode = pNode;
    if (m_pReferenceNode)
    {
        SetDlgItemText(hParams, IDC_SEL_OBJECTNAME, m_pReferenceNode->GetName());
        //EnableDlgItem(hParams, IDC_RESET_TRANSFORM, TRUE);

        //Mesh* pMesh;
        //Matrix3 mat;
        //GetMeshFromNode(pNode, 0, pMesh, mat);
        //ExtractMeshData(pMesh, mat);
    }
}

void VertexPaint::ResetTransform()
{
    ModContextList list;
    INodeTab NodeTab;

    // Only the selected nodes will be affected
    m_ip->GetModContexts(list, NodeTab);
    
    for (int i = 0; i < NodeTab.Count(); i++)
    {
        //OutputDebugString(NodeTab[i]->GetName());
        //Matrix3 mat = NodeTab[i]->GetNodeTM(m_ip->GetTime());
        //mat.Translate
        //NodeTab[i]->SetNodeTM(m_ip->GetTime(), Matrix3::Identity);
        ResetNodeTransform(NodeTab[i], m_ip);
        //NodeTab[i]->
    }
...
void VertexPaint::RepairBoundary()
{
    ModContextList list;
    INodeTab NodeTab;
    ...
    TriObject* triRefObj = GetMeshFromNode(m_pReferenceNode, t, pMesh, mat);
    //MarkBoundary(pMesh);
    GenerateBoundary(pMesh, m_userPoint);
    if (triRefObj)
    {
    ...
    //    //mat.Translate
    //    //NodeTab[i]->SetNodeTM(m_ip->GetTime(), Matrix3::Identity);
    //    //::RepairBoundary(m_userPoint, m_pReferenceNode, NodeTab[i], m_ip);
    //    //NodeTab[i]->
        ObjectState os = NodeTab[i]->EvalWorldState(t);
...
void VertexPaint::ModifyTriObject(TriObject* tobj, TimeValue t, ModContext& mc) {
    ...
    if (!m_userPoint.empty())
    {
        SetupBoundary(mesh, m_userPoint, 0.02f);
    }
