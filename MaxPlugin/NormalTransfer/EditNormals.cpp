...
#define EDIT_NORMALS_CLASS_ID Class_ID(0x4aa52ae3, 0x0D9916EA)
...
public:
	...
	// From modifier
	ChannelMask ChannelsUsed()  {return TOPO_CHANNEL|GEOM_CHANNEL;}	// Channels used by normals
	ChannelMask ChannelsChanged() {return TOPO_CHANNEL|GEOM_CHANNEL|SUBSEL_TYPE_CHANNEL;}	// Channels normals are stored in - and we set to object level.
...
public:
  ...
  Point3 FindNearestNormal(const std::vector<CLOUD>& clouds, int index, const Point3& pos, const Point3& nor, float fReferenceRadius);
  bool ReferenceNormals(INode* pNode, float fReferenceRadius);
...
static PickNormalRef sPickCB;

BOOL PickNormalRef::Filter(INode *node)
{
  return TRUE;
  //if(node) {
...
BOOL PickNormalRef::Pick(IObjParam *ip, ViewExp *vpt)
{
  ...
  TimeValue t = 0;
  float dispLength = 0;
  mo->GetParamBlock(0)->GetValue (en_display_length, t, dispLength, FOREVER);

  mo->ReferenceNormals(node, dispLength);

  //ModContext* mc = mo->ModContextFromNode(node);
  //VertexPaintData* d = (VertexPaintData*)mc->localData;
  //MeshNormalSpec* pNormals = d->GetMesh()->GetSpecifiedNormals();
  //pNormals->
  //RVertex
  //d->SetColor()
  //assert(node);
...
void PickNormalRef::EnterMode(IObjParam *ip)
{
  ICustButton *iBut = GetICustButton(GetDlgItem(mo->m_hWnd, IDC_PICKOBJ));
  iBut->SetCheck(TRUE);
  ReleaseICustButton(iBut);
}

void PickNormalRef::ExitMode(IObjParam *ip)
{
  ICustButton *iBut = GetICustButton(GetDlgItem(mo->m_hWnd, IDC_PICKOBJ));
  iBut->SetCheck(FALSE);
  ReleaseICustButton(iBut);
...
public:
	...
	void AllocateTriNormals()
  {
    if (!mpLocalTriNormals) {
      mpLocalTriNormals = new MeshNormalSpec;
    }
  }
...
INT_PTR EditNormalProc::DlgProc (TimeValue t, IParamMap2 *map,
    ...
    mpMod->m_hWnd = hWnd;
		// CAL-03/21/03: average normals
		pBtn = GetICustButton(GetDlgItem(hWnd, IDC_EN_TARGET_AVERAGE));
		pBtn->SetType(CBT_CHECK);
		pBtn->SetHighlightColor(GREEN_WASH);
    ...
    pBtn = GetICustButton(GetDlgItem(hWnd, IDC_PICKOBJ));
    pBtn->SetType(CBT_CHECK);
    pBtn->SetHighlightColor(GREEN_WASH);
    ReleaseICustButton(pBtn);
...
EditNormalsMod::EditNormalsMod()
  : m_hWnd(NULL)
  , mSelLevel(EN_SL_NORMAL)
  , mpParamBlock(NULL)
  , mShowEndResult(false)
  , mpCopiedNormal(NULL),
  mHitLevelOverride(0)
{
...
{
  editNormalSelLevel desiredLevel = selLevel ? EN_SL_NORMAL : EN_SL_OBJECT;
  if(selLevel) {
...
void EditNormalsMod::ModifyTriObject (TimeValue t, EditNormalsModData *pData, TriObject *tobj) {
	...
	if (!pData->GetTriNormals()) {
		...
		if (pPipeNormals) {
      pData->GetTriNormals()->CopySpecified (*(pPipeNormals));
    }
...
void EditNormalsMod::ActivateSubobjSel(int level, XFormModes& modes) {
	...
	mSelLevel = (editNormalSelLevel)level;
...
IOResult EditNormalsMod::Save(ISave *isave) {
	...
	res = isave->Write((int*)&mSelLevel, sizeof(mSelLevel), &nb);
	isave->EndChunk();
...
IOResult EditNormalsMod::Load(ILoad *iload) {
	...
	while (IO_OK==(res=iload->OpenChunk())) {
		...
		case kModChunkSelLevel:
			iload->Read((int*)&mSelLevel, sizeof(mSelLevel), &nb);
			break;
...
bool EditNormalsMod::PickObject()
{
  if(mpInterface == NULL) {
    return false;
  }
  ...
  sPickCB.mo = this;
  mpInterface->SetPickMode(&sPickCB);

  return true;
}

bool CalcDist(const Point3& v, const Point3& n, const Point3& pos, const Point3& nor, float* ang, float* len, float* dist)
{
  if(DotProd(n, nor) < 0)
  {
    *len = FLT_MAX; // float.MaxValue;
    *dist = FLT_MAX; // float.MaxValue;
    ang = 0;
    return false;
  }

  Point3 d = pos - v;
  float dq = d.LengthSquared();
  float a = DotProd(d, n.Normalize());
  *ang = DotProd(d.Normalize(), n.Normalize());
  *len = d.LengthSquared();
  *dist = (dq - a * a);
  return true;
}
...
struct PT
{
  Point3 pos;
  Point3 nor;
  float len;  // 采样点与参考点的距离平方
  float dist; // 采样点到参考射线的距离平方
  float ang;
  int n;

  PT(const Point3& _pos, const Point3& _nor)
    : pos(_pos)
    , nor(_nor)
    , len(0)
    , dist(0)
  ...
  //public void CalcDist(Point3 p)
  //{
  //  len = CalcDist(pos, nor, p, out dist);
  //}

  bool CalcDist(const Point3& p, const Point3& n)
  {
    return ::CalcDist(pos, nor, p, n, &ang, &len, &dist);
  }
};

typedef std::vector<REFVERTICES> GenMesh;


bool BuildRefMesh(GenMesh& ref_mesh, INode* pNode, Mesh* pMaxMesh, MeshNormalSpec* pMeshNormals)
{
  REFVERTICES v;
  int face_count = pMaxMesh->getNumFaces();
  if(face_count != pMeshNormals->GetNumFaces()) {
    return false;
  }

  Matrix3 tm = pNode->GetNodeTM(0);
  ref_mesh.insert(ref_mesh.end(), pMeshNormals->GetNumNormals(), v);
  for(int face = 0; face < face_count; face++)
  {
    for(int corner = 0; corner < 3; corner++)
    {
      int index = pMeshNormals->GetNormalIndex(face, corner);
      ref_mesh[index].n = pMeshNormals->GetNormal(face, corner);
      ref_mesh[index].v = pMaxMesh->verts[pMaxMesh->faces[face].getVert(corner)];
      ref_mesh[index].wn = tm.VectorTransform(ref_mesh[index].n);
      ref_mesh[index].wv = tm.PointTransform(ref_mesh[index].v);
    }
  }
  return true;
}

bool BuildRefMesh(GenMesh& ref_mesh, INode* pNode, MNMesh* pMaxMesh, MNNormalSpec* pMeshNormals)
{
  REFVERTICES v;
  int face_count = pMaxMesh->FNum();
  if(face_count != pMeshNormals->GetNumFaces()) {
    return false;
  }
  ...
  for(int face = 0; face < face_count; face++)
  {
    for(int corner = 0; corner < pMaxMesh->f[face].deg; corner++)
    {
      int index = pMeshNormals->GetNormalIndex(face, corner);
      ref_mesh[index].n = pMeshNormals->GetNormal(face, corner);
      ref_mesh[index].v = pMaxMesh->v[pMaxMesh->f[face].vtx[corner]].p;
      ref_mesh[index].wn = tm.VectorTransform(ref_mesh[index].n);
      ref_mesh[index].wv = tm.PointTransform(ref_mesh[index].v);
    }
  }
...
void GenerateCloudTable(std::vector<CLOUD>& clouds, const GenMesh& mesh_ref, const GenMesh& mesh)
{
  //for(int r = 0; r < mesh_ref.size(); r++)
  for(auto r : mesh_ref)
  {
    CLOUD c;
    c.pos = r.wv;
    c.nor = r.wn;
    c.dist = (c.pos - mesh[0].wv).LengthSquared();
    c.index = 0;

    for(int v = 1; v < mesh.size(); v++)
    {
      float dist = (c.pos - mesh[v].wv).LengthSquared();
      if(dist < c.dist)
      {
        c.dist = dist;
        c.index = v;
...
Point3 EditNormalsMod::FindNearestNormal(const std::vector<CLOUD>& clouds, int index, const Point3& pos, const Point3& nor, float fReferenceRadius)
{
  float dist, len, ang;
  const float epsilon = 1e-3f * fReferenceRadius * fReferenceRadius;
  //PT sel(clouds[0].pos, clouds[0].nor);
  //sel.CalcDist(pos, nor);

  //for(int i = 1; i < clouds.size(); i++)
  //{
  //  if(CalcDist(clouds[i].pos, clouds[i].nor, pos, nor, &ang, &len, &dist) && len < sel.len)
  //  {
  //    sel.pos = clouds[i].pos;
  //    sel.nor = clouds[i].nor;
  //    sel.len = len;
  //    sel.dist = dist;
  //    sel.n = i;
  //  }
  ...
  Point3 normal = nor * fReferenceRadius * 0.01f;

  //for(int i = 0; i < clouds.size(); i++)
  for(auto cloud : clouds)
  {
    if(cloud.dist < epsilon)
    {
    ...
    else if(CalcDist(cloud.pos, cloud.nor, pos, nor, &ang, &len, &dist))
    {
      normal += cloud.nor * (1.0f / (exp(len / (fReferenceRadius * fReferenceRadius))));
    }
  }
...
bool EditNormalsMod::ReferenceNormals(INode* pNode, float fReferenceRadius)
{
  const bool useThresh = false;
  ...
  if(pObject->CanConvertToType(Class_ID(TRIOBJ_CLASS_ID, 0)))
  {
    ...
    int num = mesh.numVerts;
    //mesh.getNormal();

    MeshNormalSpec* pMeshNormals = (MeshNormalSpec *)mesh.GetSpecifiedNormals();
    MeshNormalSpec* pRefNormals = new MeshNormalSpec();
    if(pMeshNormals == NULL) {
      return false;
    }
    
    pRefNormals->CopySpecified(*pMeshNormals);
    pRefNormals->SetParent(&mesh);
    pRefNormals->CheckNormals();
    pRefNormals->SetParent(NULL);


    int mesh_face_cnt = mesh.getNumFaces();
    int normal_face_cnt = pRefNormals->GetNumFaces();
    if(mesh_face_cnt != normal_face_cnt)
    {
      return false;
    }

    BuildRefMesh(ref_mesh, pNode, &mesh, pRefNormals);
  ...
  for(int i = 0; i < list.Count(); i++)
  {
    ...
    if(pData->GetFlag (ENMD_TRIMESH)) {
      ...
      pData->GetTriNormals()->SetParent(pData->GetTriMeshCache());
      pData->GetTriNormals()->CheckNormals();
      //int face_count = pData->EnfnGetTriMesh(0)->getNumFaces();
      //for(int face = 0; face < face_count; face)
      //{
      //}

      BuildRefMesh(my_mesh, nodes[i], pData->GetTriMeshCache(), pData->GetTriNormals());
      GenerateCloudTable(clouds, ref_mesh, my_mesh);

      Matrix3 mt = nodes[i]->GetNodeTM(0);
      mt.Invert();

      //for(size_t i = 0; i < my_mesh.size(); i++)
      int index = 0;
      for(REFVERTICES& vp : my_mesh)
      {
        Point3 normal = FindNearestNormal(clouds, index, vp.wv, vp.wn, fReferenceRadius);
        vp.wn = normal;
        vp.n = mt.VectorTransform(normal);
        index++;
      }

      int num = pData->GetTriNormals()->GetNumNormals();
      for(int n = 0; n < num; n++)
      {
        pData->GetTriNormals()->Normal(n) = my_mesh[n].n;
      }
      localChange = true;

      //if(pData->GetTriNormals()->Normal()
      //  AverageNormals(useThresh, threshold, mSelLevel)) {
      //  pData->InvalidateTopoCache();
      //  localChange = true;
      //}
      ...
      // Can't leave it connected, either:
      pData->GetTriNormals()->SetParent(NULL);
      pData->GetTriNormals()->MakeNormalsExplicit(false, NULL, true);
    }
    else {
      if(!pData->GetPolyMeshCache()) {
        DbgAssert (0);
      ...
      BuildRefMesh(my_mesh, nodes[i], pData->GetPolyMeshCache(), pData->GetPolyNormals());
      GenerateCloudTable(clouds, ref_mesh, my_mesh);
      ...
      for(REFVERTICES& vp : my_mesh)
      {
        Point3 normal = FindNearestNormal(clouds, i, vp.wv, vp.wn, fReferenceRadius);
        vp.wn = normal;
        vp.n = mt.VectorTransform(normal);
      }

      int num = pData->GetPolyNormals()->GetNumNormals();
      for(int n = 0; n < num; n++)
      {
        pData->GetPolyNormals()->Normal(n) = my_mesh[n].n;
      }


      localChange = true;

      //if(pData->GetPolyNormals()->GetNormal()
      //  AverageNormals(useThresh, threshold, mSelLevel)) {
      //  pData->InvalidateTopoCache();
      //  localChange = true;
      //}
      ...
      // Can't leave it connected, either:
      pData->GetPolyNormals()->SetParent(NULL);
      pData->GetPolyNormals()->MakeNormalsExplicit(false, NULL, true);
    }
    ...
    if(localChange) {
      ...
      //  // Macrorecord each node separately:
      //  if(useThresh) {
      //    macroRecorder->FunctionCall(_T("$.modifiers[#Edit_Normals].EditNormalsMod.Average"), 0, 3, _T("useThresh"), mr_bool, useThresh, _T("threshold"), mr_float, threshold, _T("node"), mr_reftarg, nodes[i]);
      //  }
      //  else {
      //    macroRecorder->FunctionCall(_T("$.modifiers[#Edit_Normals].EditNormalsMod.Average"), 0, 1, _T("node"), mr_reftarg, nodes[i]);
      //  }
      //}
      //else {
      //  // Macrorecord with no node specified:
      //  if(useThresh) {
      //    macroRecorder->FunctionCall(_T("$.modifiers[#Edit_Normals].EditNormalsMod.Average"), 0, 2, _T("useThresh"), mr_bool, useThresh, _T("threshold"), mr_float, threshold);
      //  }
      //  else {
