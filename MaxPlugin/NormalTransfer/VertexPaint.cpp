...
{
  ICustButton *iBut = GetICustButton(GetDlgItem(mo->hParams, IDC_PICKOBJ));
  iBut->SetCheck(TRUE);
  ...
  HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_INITDIALOG:
    ...
    mod->iPickObjButton = GetICustButton(GetDlgItem(hWnd, IDC_PICKOBJ));
    mod->iPickObjButton->SetType(CBT_CHECK);
    mod->iPickObjButton->SetHighlightColor(GREEN_WASH);
    //mod->iPickObjButton->SetCheck(mod->ip->GetCommandMode()->ID() == CID_PAINT &&
    //  ((PaintMouseProc *)mod->ip->GetCommandMode()->MouseProc(&numPoints))->GetPickMode());
...
ICustButton* VertexPaint::iPickObjButton = NULL;
