...
struct DRAWPRIMITIVECALL
{
  ...
  DRAWPRIMITIVECALL(
    ...
    LPDIRECT3DBASETEXTURE9 _pBaseTexture)
    : pIndexBuffer(_pIndexBuffer)
    , pBaseTexture(_pBaseTexture)
  {}

  bool operator<(const DRAWPRIMITIVECALL& v) const
  {
    if (pIndexBuffer == v.pIndexBuffer) {
      return pBaseTexture < v.pBaseTexture;
    }
    return pIndexBuffer < v.pIndexBuffer;
  }
};
...
{
  int nMeshIndex;
  int aTextureIndex[MAX_TEXTURE_SLOT];
};
...
struct MyIDirect3DDevice9 : public IDirect3DDevice9
{
  IDirect3DDevice9* m_pD3DDevice9;
  clFile m_file;
  DWORD m_FVF;

  IDirect3DVertexShader9* m_pVertexShaderUnsafe;
  IDirect3DPixelShader9* m_pPixelShaderUnsafe;
  IDirect3DIndexBuffer9* m_pIndexBufferUnsafe;
  IDirect3DVertexBuffer9* m_pVertexBufferUnsafe[16];
  IDirect3DVertexDeclaration9* m_pVertexDeclarationUnsafe;
  clmap<DRAWPRIMITIVECALL, DRAWPRIMITIVEDESC> m_sDrawCallMap;
  clmap<IDirect3DVertexShader9*, int> m_sVertexShaderMap;
  clmap<IDirect3DPixelShader9*, PIXELSHADERCACHE> m_sPixelShaderMap;
  IDirect3DBaseTexture9* m_pTexturesUnsafe[MAX_TEXTURE_SLOT];
  float4 m_aVertexConstantBuffer[256];
  float4 m_aPixelConstantBuffer[256];
  int m_nDumpMeshIndex;


  MyIDirect3DDevice9();
  virtual ~MyIDirect3DDevice9();

  DWORD IntGetTextureSlotMask();
...
struct MyIDirect3DDevice9Ex : public IDirect3DDevice9Ex
{
  IDirect3DDevice9Ex* m_pD3DDevice9;
  clFile m_file;
  DWORD m_FVF;
  ...
  STDMETHOD(CreateVolumeTexture)(THIS_ UINT Width, UINT Height, UINT Depth, UINT Levels,
    DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, IDirect3DVolumeTexture9** ppVolumeTexture, HANDLE* pSharedHandle);

  STDMETHOD(CreateCubeTexture)(THIS_ UINT EdgeLength, UINT Levels, DWORD Usage,
    D3DFORMAT Format, D3DPOOL Pool, IDirect3DCubeTexture9** ppCubeTexture, HANDLE* pSharedHandle);
