...
class IMyD3D11DeviceContext : public ID3D11DeviceContext
{
  ID3D12Device* m_pd3d12Device = nullptr;
  ULONG m_nRefCount = 0;
#if 1
  ID3D12CommandAllocator* m_pd3d12CommandAlloc = nullptr;
  ID3D12GraphicsCommandList* m_pd3d12CommandList = nullptr;
  ID3D12Fence* m_pFence = nullptr;
  ID3D12Resource* m_pScratchResource = nullptr; // 这个虽然有引用计数管理，但是在命令执行完毕前不能释放
  LONG_PTR m_nRowPitch = 0;
  LONG_PTR m_nSlicePitch = 0;
#endif
  HANDLE m_hFenceEvent = nullptr;
  UINT64 m_uFenceValue = 1;
public:
  IMyD3D11DeviceContext(ID3D12Device* pd3d12Device);    

  virtual ~IMyD3D11DeviceContext( );
...
ID3D11CommandList** ppCommandList) override;
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  void Upload(ID3D12Resource* resource, uint32_t subresourceIndexStart, const D3D12_SUBRESOURCE_DATA* subRes, uint32_t numSubresources);
