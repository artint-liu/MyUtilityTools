...
# 思考语言锚定：每次调用 LLM 前附加到上下文最末（紧邻生成起点）。
# 推理模型的语言极易被最近的输入带偏——工具结果多为英文（代码/日志/JSON），
# 不锚定时常出现"中文思考 → 工具调用 → 英文思考"的漂移。
ZH_THINKING_ANCHOR = "\n\n（注意：接下来的思考过程与回复请务必使用简体中文。）"

_BASE_PROMPT = """你是 clagent，一个面向 Unity 开发的 AI Agent，通过 MCP（Model Context Protocol）服务器与 Unity 编辑器通信。
请全程使用简体中文（包括思考 / 推理过程），代码与标识符保持英文。
...
- 语言要求（重要）：思考 / 推理过程（思维链）与最终回复一律使用简体中文，不要用英文思考；仅代码、标识符、路径、原始报错信息保持英文原文。
